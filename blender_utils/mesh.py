...
from blender_utils.modifier import add_subdivision_surface_modifier
...
def create_three_smooth_sphere(radius = 1.0,
        names: Optional[Tuple[str, str, str]] = None) -> Tuple[bpy.types.Object, bpy.types.Object, bpy.types.Object]:
    if names is None:
        names = ("Ball Left", "Ball Center1", "Ball Center2", "Ball Right")

    offset = radius / 5

    left = create_smooth_sphere(location=(-(radius+offset)*3, 0.0, radius), radius=radius, name=names[0])
    center1 = create_smooth_sphere(location=(-(radius+offset), 0.0, radius), radius=radius, name=names[1])
    center2 = create_smooth_sphere(location=(+(radius+offset), 0.0, radius), radius=radius, name=names[2])
    right = create_smooth_sphere(location=(+(radius+offset)*3, 0.0, radius), radius=radius, name=names[3])

    return left, center1, center2, right
